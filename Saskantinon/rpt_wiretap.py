...
class WireTap(object):
    ...
    When a message is logged, it is written as a discrete file
    to the log directory only if the message level is within scope of
    currently-configured log level.

    @DEV:
    - Consider using services instead of direct calls.
      Or some kind of promises.
    - Break out generic services to saskan_methods
    """
