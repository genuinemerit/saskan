...
class Graphs(object):
    ...
    # Helper functions
    def get_ntype(self, p_set: str, p_node_nm: str):
    ...
    def get_data(self, p_file_path: str, p_sheet_nm: str = None) -> DataFrame:
        ...
        if ss_type.lower() in ("xlsx", "xls"):
            sheet_df = pd.read_excel(p_file_path, sheet_name=p_sheet_nm)
        elif ss_type.lower() in ("ods"):
            sheet_df = pd.read_excel(p_file_path, engine="odf", sheet_name=p_sheet_nm)
    ...
    def set_source(self, p_file_name: str, p_sheet_nm: str = None) -> tuple:
        """
    ...
    def get_ss_data(self, p_file_name: str, p_sheet_nm: str = None):
        """
        Input file is Excel, ODS, comma-separated (csv), tab-separated (tsv),
        or tilde-separated (txt). If Excel or ODS, then a sheet name needs
        to be supplied also.
        ...
        self.DS[dataset_nm] = self.get_data(path.join(src_dir, p_file_name), p_sheet_nm)

    def get_json_data(self, p_set: str):
        """
        ...
        self.E[p_set] = {"types": list(), "name": dict(), "color": dict()}
        self.N[p_set]["types"] = sorted(list(set(self.DS[p_set][0].keys())))

    def set_node_names(self, p_set: str):
        ...
        for nix, ntyp in enumerate(self.N[p_set]["types"]):
            self.N[p_set]["name"][ntyp] = sorted(list(set(self.N[p_set]["name"][ntyp])))
    ...
    def set_edge_types(self, p_set: str, p_no_edge: list = [("", "")]):
        ...
        for n1 in self.N[p_set]["types"]:
            for n2 in [n for n in self.N[p_set]["types"] if n != n1]:
                ...
                for en1, en2 in p_no_edge:
                    if (n1 == en1 and n2 == en2) or (n1 == en2 and n2 == en1):
                        skip_it = True
                        break
                if not skip_it:
    ...
    def set_edges(self, p_set: str):
        ...
        for eix, etyp in enumerate(self.E[p_set]["types"]):
            ...
            self.E[p_set]["name"][etyp] = sorted(list(set(self.E[p_set]["name"][etyp])))
            # colors
    ...
    def analyze_data(self, p_set: str):
        ...
        pp(
            (
                AL.get_edges_for_nodes(
                    self.E[p_set],
                    ("people", "scene"),
                    ["Thinker Stanley P. Quinn", "Magister Showan of the Nywing"],
                )
            )
        )
        pp(
            (
                AL.get_edges_for_nodes(
                    self.E[p_set],
                    ("scene", "people"),
                    ["001 Full Moons Rising", "003 Full Moons Waning"],
                )
            )
        )
